...
parser.add_argument(
    "-i", "--input_file", help="Path to a csv file containing data for ingestion."
)
parser.add_argument(
    "-fg", "--feature_group_name", help="Name of the feature group to write data to."
)
...
with open(args.input_file) as file_handle:
    for row in csv.DictReader(file_handle, skipinitialspace=True):
        ...
        for featureName, valueAsString in row.items():
            record.append({"FeatureName": featureName, "ValueAsString": valueAsString})
        sagemaker_featurestore_runtime_client.put_record(
            FeatureGroupName=args.feature_group_name, Record=record
        )
